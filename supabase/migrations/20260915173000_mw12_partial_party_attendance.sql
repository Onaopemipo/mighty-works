-- MW-12A — Partial party attendance foundation.
--
-- party_size:
--   total people covered by one registration.
--
-- checked_in_count:
--   number of people currently present.
--
-- Legacy compatibility:
--   checked_in = checked_in_count > 0
--   checked_in_at = first arrival in current attendance cycle.
...
set checked_in_count =
  case
    when coalesce(checked_in, false)
      then greatest(
        coalesce(party_size, 1),
        1
      )
    else 0
  end
where checked_in_count is null;

alter table public.registrations
  alter column checked_in_count
  set default 0;
...
alter table public.registrations
  drop constraint if exists
    registrations_checked_in_count_check;
